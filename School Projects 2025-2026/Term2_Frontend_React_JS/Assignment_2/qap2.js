...
window.addEventListener("DOMContentLoaded", function () {
  ...
  let problem1Button = document.querySelector("#btn1");

  function snake(value) {
    const trimmed = value.trim();                             // Remove any whitespace at the start/end
    const lowerCased = trimmed.toLowerCase();                 // convert to lowercase
    const replaced = lowerCased.replace(/[^a-z0-9\s]/g, " "); // replace any non-number/letter characters with a space
    const cleaned = replaced.replace(/\s+/g, "_");            // replaces any whitespace with underscore

    return cleaned;    
  }

  problem1Button.addEventListener("click", function () {    // when button is clicked, run snake function
    const input = document.querySelector("#input1").value;
    const output = snake(input);                              // converts input
    document.querySelector("#p1").textContent = output;
  });
  ...
  let problem2Button = document.querySelector("#btn2");

  problem2Button.addEventListener("click", function () {
    const src = document.querySelector("#input2").value;
    const width = document.querySelector("#width").value;
    const controls = document.querySelector("#controls").checked;

    createVideo(src, width, controls);
  });

  function createVideo(src, width, controls) {
    let trimmedSrc = src.trim();                                // Remove leading/trailing whitespace from src
    let videoElement = `<video src="${trimmedSrc}" autoplay`;   // creates the video src element, autoplay regardless of controls
  
    let isValidURL = /^(https?:\/\/)[^\s]+$/i.test(trimmedSrc); // validates the URL format

    if (!isValidURL) {
      document.querySelector("#videoembed").innerHTML = `<p style="color: red;">Invalid video URL</p>`;
      return;
    }
    ...
    if (width && !isNaN(width)) {
      ...
      width = "100%";                                 // If width is not provided or is not a number, default to 100%
      videoElement += ` width="${width}" `
      document.innerHTML = `<p style="color: red;">Invalid width, defaulting to 100%</p>`;
    }

    if (controls) {
      videoElement += ` controls`;                    // If controls = true, add controls to video src
    }

    videoElement += ` height="300px"`;                // set height for auto scaling into the grid box
    videoElement += `></video>`;                      // Closing the tag

    document.querySelector("#videoembed").innerHTML = videoElement;

    // I was trying to get it to also work with youtube links, but it was crazy complicated, so I just left it out
  }
  ...
  let problem3Button = document.querySelector("#btn3");

  problem3Button.addEventListener("click", function () {
    // Grab the input value and output element
    const dateInput = document.querySelector("#input3").value;
    const outputElement = document.querySelector("#poutput3");

    try {
      const parsedDate = parseDateString(dateInput);
      outputElement.innerText = `Parsed Date: ${parsedDate.toDateString()}`;
      outputElement.style.color = "green";
    } catch (error) {
      outputElement.innerText = error.message;    // display given error message
  ...
  function parseDateString(dateString) {
    ...
    if (!dateFormat.test(dateString)) {
      throw new Error("Date format must be exactly YYYY-MM-DD.");
    }

    const [yearStr, monthStr, dayStr] = dateString.split("-");
    const year = parseInt(yearStr, 10);                         // the 10 simply tells JS to interpret the number as a base 10 number
    const month = parseInt(monthStr, 10);
    const day = parseInt(dayStr, 10);

    // Enforce digit lengths
    if (yearStr.length !== 4) {
    ...
    // Construct and return date using set methods
    const date = new Date();
    date.setFullYear(year);
    date.setMonth(month - 1); // months are indexed from 0, so Feb is 1 not 2
    date.setDate(day);
  ...
  let problem4Button = document.querySelector("#btn4");

  problem4Button.addEventListener("click", function () {
    const dateInput = document.querySelector("#input4").value;
    const outputElement = document.querySelector("#poutput4");

    try {
      const newDate = new Date(dateInput);
      const formatted = toDateString(newDate);
      outputElement.innerText = `Converted Date String: ${formatted}`;
    } catch (error) {
      outputElement.innerText = error.message;  // display given error message
      outputElement.style.color = "red";
    }
  ...
  function toDateString(value) {
    ...
    const month = String(value.getMonth() + 1).padStart(2, '0');  // padStart(2, '0') ensures month is 2 digits, and adds the zero if not
  ...
  let problem5Button = document.querySelector("#btn5");

  problem5Button.addEventListener("click", function () {
    const inputLat = document.querySelector("#inputLat1").value;
    const inputLong = document.querySelector("#inputLng1").value;
    const outputCoord = document.querySelector("#poutput5");

    try {
      const lat = normalizeCoord(inputLat, "Latitude");     // adding the type of coordinate for customizing the error messages and checks
      const lng = normalizeCoord(inputLong, "Longitude");

      outputCoord.innerText = `(${lat}, ${lng})`;
    } catch (error) {
      outputCoord.innerText = error.message;
      outputCoord.style.color = "red";
    }
  });

  function normalizeCoord(value, type) {
    // initial checks for value type and format
    if (value === null || typeof value !== 'string' || value.trim() === '' || isNaN(Number(value))) { // value.trim() === '' checks for empty string of spaces
      throw new Error(`${type} must be a number.`);
    }

    value = value.replace(/[\[\]\s]/g, ""); // remove brackets and whitespace if necessary

    const number = parseFloat(value);       // floats both input values while retaining their type
    if (type === "Latitude" && (number < -90 || number > 90)) {
      throw new Error("Latitude must be between -90 and 90.")
    }

    if (type === "Longitude" && (number < -90 || number > 90)) {
      throw new Error("Longitude must be between -90 and 90.")
    }

    return number.toFixed(3); // format the coordinates to 3 decimal places for easier readability, coordinates can go up to 6 decimal places
  }
  ...
  ******************************************************************************/
  ...
  let problem6Button = document.querySelector("#btn6");
  const coordOutput = document.querySelector("#poutput6");

  let allCoords = [];

  function formatCoords(...coordStrings) {
    const coordList = [];
    ...
    coordStrings.forEach(coordStr => {
      try {
        // replace brackets and whitespace
        let cleaned = coordStr.replace(/[\[\]\s]/g, "");

        // split coordinates for manipulation
        const [latRaw, lngRaw] = cleaned.split(",");

        // Ensures both lat and long are provided
        if (!latRaw || !lngRaw) throw new Error("Invalid coordinate format");

        // ensure no leading/triling whitespace
        const lat = latRaw.trim();
        const lng = lngRaw.trim();

        const normLat = parseFloat(lat);
        const normLng = parseFloat(lng);

        if (isNaN(normLat) || isNaN(normLng)) throw new Error("Invalid number");

        coordList.push(`(${normLat}, ${normLng})`);
      } catch {
  ...
  problem6Button.addEventListener("click", function() {
    const inputCoords = document.querySelector("#inputCoords").value;

    const inputs = inputCoords.split(/;|\n/).map(s => s.trim()).filter(s => s.length > 0);

    allCoords.push(...inputs);

    const formatted = formatCoords(...allCoords);
    coordOutput.innerText = formatted;
  });
  ...
  ******************************************************************************/
  ...
  let problem7Button = document.querySelector("#btn7");
  const input7 = document.querySelector("#input7");
  const output7 = document.querySelector("#poutput7");
  ...
  function mimeFromFilename(filename) {
    ...
    const extension = filename.slice(lastDot + 1).toLowerCase(); // need +1 to target the last period as the slice point

    // Switch statements to determine MIME type
    switch (extension) {
      case "html":
      case "htm":
        return "text/html";
  ...
  problem7Button.addEventListener("click", function () {
    const filename = input7.value.trim();

    if (filename === "") {
      return output7.innerHTML = "Please enter a filename.";
    }

    const mimeType = mimeFromFilename(filename);

    output7.textContent = `MIME Type: ${mimeType}`;
  });
  ...
  let problem8Button = document.querySelector("#btn8");
  const copyrightInput = document.querySelector("#input8");
  const linkOutput = document.querySelector("#poutput8");
  const newTabCheckbox = document.querySelector("#newTabCheck"); // Get checkbox element

  problem8Button.addEventListener("click", function () {
    const licenseCode = copyrightInput.value.trim();

    if (licenseCode === "" || licenseCode.length < 5) {
      linkOutput.innerHTML = "Please enter a license code.";
      linkOutput.style.color = "red";
      return;
    }

    const openInNewTab = newTabCheckbox.checked; // true if checkbox is checked
    const result = generateLicenseLink(licenseCode, openInNewTab);
    linkOutput.innerHTML = result;
  });

  function generateLicenseLink(licenseCode, targetBlank) {
    const code = licenseCode.trim().toUpperCase();
    const shortCode = code.startsWith("CC-") ? code.slice(3).toLowerCase() : code.toLowerCase();

    let licenseText = "";
    let newCC = shortCode;
    let url = `https://creativecommons.org/licenses/${newCC}/4.0/`;

    if (shortCode === "by") {
      licenseText = "Creative Commons Attribution License";
    } else if (shortCode === "by-nc") {
      licenseText = "Creative Commons Attribution-NonCommercial License";
    } else if (shortCode === "by-sa") {
      licenseText = "Creative Commons Attribution-ShareAlike License";
    } else if (shortCode === "by-nd") {
      licenseText = "Creative Commons Attribution-NoDerivs License";
    } else if (shortCode === "by-nc-sa") {
      licenseText = "Creative Commons Attribution-NonCommercial-ShareAlike License";
    } else if (shortCode === "by-nc-nd") {
    ...
    return `<a href="${url}"${targetAttribute}>${licenseText}</a>`;
  }
  ...
  let problem9Button = document.querySelector("#btn9");
  const booleanInput = document.querySelector("#input9");
  const verifyOutput = document.querySelector("#poutput9");

  problem9Button.addEventListener("click", function () {
    try {
      const result = pureBool(booleanInput.value); // use input value directly
      verifyOutput.innerHTML = `This value evaluates to: <strong>${result}</strong>`;
    } catch (error) {
      verifyOutput.innerHTML = error.message;
  ...
  function pureBool(value) {
    ...
    const normalized = String(value).trim(); // keep raw input case sensitive

    const trueValues = [
      "Yes", "yes", "YES", "Y", "y", "Oui", "oui", "OUI", "O", "o", "t", "T", "TRUE", "true", "True", "vrai", "Vrai", "VRAI", "V", "v"
    ];

    const falseValues = [
      "No", "no", "NO", "N", "n", "Non", "non", "NON", "f", "F", "FALSE", "false", "False", "FAUX", "faux", "Faux"
    ];

    // Check numerica value is positive or negative for true/false
   ...
   * any('Y', 'no', 1) --> returns partially true
  ...
  let problem10Button = document.querySelector("#btn10");
  const booleanListInput = document.querySelector("#input10");
  const verifyListOutput = document.querySelector("#poutput10");

  problem10Button.addEventListener("click", function () {
    const input = booleanListInput.value;

    // Split list input by comma, trim whitespace
    const items = input.split(",").map(item => item.trim());

    const allTrue = every(...items);
    const someTrue = any(...items);
    const noneTrue = none(...items);

    verifyListOutput.innerHTML = `
      <p>Every value is true? <strong>${allTrue}</strong></p>
      <p>At least one value is true? <strong>${someTrue}</strong></p>
      <p>None of the values are true? <strong>${noneTrue}</strong></p>
    `;
  });

  // Use your existing pureBool function here (must be defined above these!)
  function pureBool(value) {
    if (value === "" || value === 0 || value === undefined) {
      throw new Error("invalid value");
    }
  ...
  // Check if ALL values evaluate to true
  function every(...args) {       // when using w3/chatGPT/copilot to help with this code, I learned about args. It's a rest paramter which allows previous argument to pass through it
    let isValid = false;

    for (let item of args) {
      try {
        if (!pureBool(item)) return false;
        isValid = true;           // tbh not sure how this structure works but it's how I had to do it to work
      } catch (error) {
        continue;
      }
    }

    return isValid; // Only return true if there was at least one valid true
  }
  ...
  let problem11Button = document.querySelector("#btn11");
  const urlQuery = document.querySelector("#inputQuery");
  const urlOrder = document.querySelector("#inputOrder");
  const urlCount = document.querySelector("#inputCount");
  const urlLicense = document.querySelector("#inputLiscense");
  const urlOutput = document.querySelector("#poutput11");

  problem11Button.addEventListener("click", function () {
    try {
      const query = urlQuery.value.trim();
      const order = urlOrder.value.trim().toLowerCase();
      const count = parseInt(urlCount.value.trim());
      const license = urlLicense.value.trim().toLowerCase();

      const finalUrl = buildUrl(query, order, count, license);
      urlOutput.innerHTML = `<a href="${finalUrl}" target="_blank">${finalUrl}</a>`;
    } catch (error) {
      urlOutput.innerText = error.message;
  ...
  function buildUrl(query, order, count, license) {
    ...
    if (!validOrders.includes(order)) {   // checks for ascending/descending
      throw new Error("Order must be 'asc' for ascending or 'desc' for descending");
    }

    if (isNaN(count) || count < 1 || count > 50) {
      throw new Error("Count must be a number from 1 to 50");
    }

    if (!validLicenses.includes(license)) {   // checks for valid license
